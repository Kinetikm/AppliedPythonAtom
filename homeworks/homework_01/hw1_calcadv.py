...
# coding: utf-8
'''
      алькул€тор на основе обратной польской записи.
     –азрешенные операции: открыта€ скобка, закрыта€ скобка,
          плюс, минус, умножить, делить
     :param input_string: строка, содержаща€ выражение
     :return: результат выполнение операции, если строка валидна€ - иначе None
'''
...
def Check_valid(string):
    check_str = "\
    qwertyuiop[]';lkjhgfdsazxcvbnm,йцукенгшщзхъэждлорпавыф€чсмитьбю\n\t\
         "
    for i in string:
        if i in check_str:
            return False
