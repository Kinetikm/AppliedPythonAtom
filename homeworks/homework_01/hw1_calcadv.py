...
def advanced_calculator(input_string):
    ...
    for i in range(len(sequence) - 1):
        ...
        if isinstance(sequence[i], float) and sequence[i + 1] == '(':
            return None
        if isinstance(sequence[i + 1], float) and sequence[i] == ')':
            return None
    for i in range(len(sequence) - 1):
        if sequence[i] != '-':
            continue
        if isinstance(sequence[i + 1], float):  # '-','1' -> ' ','-1'
            sequence[i + 1] *= -1
            if i == 0 or sequence[i - 1] == '(' or sequence[i - 1] == '/':
                sequence[i] = ' '
            else:
                sequence[i] = '+'
    while ' ' in sequence:
